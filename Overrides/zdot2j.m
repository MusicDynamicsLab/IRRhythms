...
for cx = 1:length(n.con)
    ...
    switch con.nType    % cases ordered by frequency of use
        ...
        case 6  % all2freq
            if con.no11
                x = x + con.w .* sum(con.C.*( A2(e, z)*P2(e, y.') ...
                    - A2(e^2, z*y').*repmat(y.', con.targetN, 1) ), 2);
            else
                x = x + con.w .* sum(con.C.*( A2(e, z)*P2(e, y.') ), 2);
            end
            
        case 7  % allfreq
            if con.no11
                x =  x + con.w .* sum(con.C.*( A2(e, z)*P2_new(e, y.') ...
                    - A2(e^2, z*y').*repmat(y.'.*A2(e^2, abs(y.').^2), con.targetN, 1) ), 2);
            else
                x =  x + con.w .* sum(con.C.*( A2(e, z)*P2_new(e, y.') ), 2);
            end
        ...
        case 2  % 2freq
            ...
            Y2= repmat(sqrt(e)*y.', con.targetN, 1).^ (2*NUM);      %EWL 3/18/23 need to test
            Z2= repmat(sqrt(e)*conj(z) , 1, con.sourceN).^(2*DEN-1); % does this work right?
            % x = x + con.w .* sum((1/4)*con.C.*Y.*Z,2)/sqrt(e) + con.w .*
            % sum((NUM+DEN).*con.C.*Y2.*Z2,2)/sqrt(e); 
            % After Ji Chul's amalysis
            x = x + con.w .* sum(con.C.*Y2.*Z2,2)/sqrt(e); % (1.5.*(NUM+DEN-2)).*
...
y = P(epsilon,x)./(max(1,P(epsilon,abs(x))));
