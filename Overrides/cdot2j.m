...
%         X  = kappa.* abs((sqrt(e)*Z).^DEN.*(sqrt(e)*Y).^NUM/e ... % EWL 03/04/22
%                         + antiStrength*(sqrt(e)*Z).^(2*DEN).*(sqrt(e)*Y).^(2*NUM)/e)/(1+antiStrength); % EWL 03/04/22
        X  = kappa.*      abs(real((sqrt(e)*Z).^   DEN .*(sqrt(e)*Y).^   NUM /e)); % analysis with JCK 09/15/2023
        % % After Ji Chul's analysis
        % X  = kappa.*      abs(real((sqrt(e)*Z).^(2*DEN).*(sqrt(e)*Y).^(2*NUM)/e)); % analysis with JCK 09/15/2023
